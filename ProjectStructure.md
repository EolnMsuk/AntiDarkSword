...
│   │                                     # replica of AltList_New — regenerated by build scripts before each make
│   │                                     # invocation. Do not edit directly.
...
│   └── AltList_Old.framework             # AltList build for iOS 13–14 self-builds only. Linked against
│                                         # iPhoneOS14.5.sdk; must be thinned to arm64-only via lipo before use.
│                                         # Not used by CI.
│
└── .github/
    └── workflows/
        └── build.yml                     # GitHub Actions CI workflow (macos-14 runner). Installs Theos via
                                          # Randomblock1/theos-action, downloads iPhoneOS16.5.sdk, and builds
                                          # three targets: rootful arm.deb, rootless arm64.deb, TrollFools dylib
                                          # (all iOS 15–17.0, arm64 + arm64e, AltList_New). Publishes a draft
                                          # GitHub Release on every push to main. iOS 13–14 not built by CI.
```
